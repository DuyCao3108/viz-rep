...
class Line:
    ...
    def _chart_plot(self) -> "Line":
        ...
        mmax = max(
            (entry["result_vals"].max() for entry in self._mes_maps.values() if len(entry["result_vals"])),
            default=0,
        )
        self.ax.set_ylim(0, mmax * 1.18 if mmax else 1)
        return self
    ...
    def show_data_label(self) -> None:
        ...
        for entry in self._mes_maps.values():
            ...
            for xi, rv, dv in zip(x, entry["result_vals"], entry["display_vals"]):
                self.ax.annotate(
                    dv, xy=(xi, rv), xytext=(0, 8), textcoords="offset points",
    ...
    def set_theme(self, pal: Pal = "cate-55", tone: Tone | None = "minimal") -> "Line":
